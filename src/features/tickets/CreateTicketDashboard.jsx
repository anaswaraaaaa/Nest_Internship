...
export default function CreateTicketDashboard({ productsRegistry, onSaveTicket, onCancel, styles }) {
  ...
  // Controls which item's checklist checklist drawer is currently expanded/viewed
  const [expandedChecklistUid, setExpandedChecklistUid] = useState(null);

  // Core baseline checklist verification points
  ...
  const handleScanItemSubmit = (e) => {
    ...
    const freshScannedItem = {
      ...
      partType: matchedPart.partType, // Default initial value for row dropdown
      selectedCheckpoints: [...baselineChecklistOptions], // Default all ticked initially
      status: "Verified",
      remarks: ""
    };

    setStagedItems([...stagedItems, freshScannedItem]);
    setScannedModelInput(""); // Reset scanner register field
  };

  // Inline updater rules for row-specific values
  const updateRowProperty = (uid, property, value) => {
    setStagedItems(prev => prev.map(item => {
      ...
      // Dynamic logic: If any checklist item is unchecked, automatically flag status as "Rejected"
  ...
  const handleCommitFinalBatch = () => {
    ...
    // Convert mapping names back to master schema variables seamlessly
    const structuredItems = stagedItems.map(i => ({
      ...i,
      qcChecked: i.status === "Verified",
      checklist: i.status
    }));
    onSaveTicket(workOrderNo.trim(), structuredItems);
  };
  ...
  return (
    <div style={styles.panel}>
      ...
      {/* Primary Work Order Parameter Row */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "24px", marginBottom: "24px", alignItems: "end" }}>
        ...
        <form onSubmit={handleScanItemSubmit} style={{ display: "flex", gap: "12px", width: "100%" }}>
          <div style={{ flexGrow: 1 }}>
            ...
            <input 
              type="text" 
              placeholder="Type model code (e.g. ITM-902) and press Enter..." 
              value={scannedModelInput} 
              onChange={(e) => setScannedModelInput(e.target.value)} 
              style={styles.formInlineInput} 
            />
          </div>
          <button type="submit" style={{ ...styles.submitButton, height: "40px", marginTop: "21px" }}>
            Scan Component
          </button>
        </form>
      </div>
                ...
                <tr style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <input 
                      ...
                      min="1" 
                      value={item.qty} 
                      onChange={(e) => updateRowProperty(item.uid, "qty", parseInt(e.target.value) || 1)} 
                      style={{ ...styles.formInlineInput, width: "70px", padding: "6px" }} 
                    />
                  </td>
                  <td style={styles.td}>
                    <img 
                  ...
                  <td style={styles.td}>
                    <select 
                      value={item.partType} 
                      onChange={(e) => updateRowProperty(item.uid, "partType", e.target.value)} 
                      style={{ ...styles.formInlineInput, padding: "6px" }}
                    >
                  ...
                  <td style={styles.td}>
                    <button 
                      type="button" 
                      onClick={() => setExpandedChecklistUid(expandedChecklistUid === item.uid ? null : item.uid)}
                      style={{
                        ...styles.actionInlineBtn,
                        backgroundColor: expandedChecklistUid === item.uid ? "#0f172a" : "#f8fafc",
                        color: expandedChecklistUid === item.uid ? "#ffffff" : "#1a3a8f"
                      }}
                    >
                  ...
                  <td style={styles.td}>
                    <span style={{ 
                      ...styles.badge, 
                      backgroundColor: item.status === "Verified" ? "#f0fdf4" : "#fff5f5", 
                      color: item.status === "Verified" ? "#16a34a" : "#e11d48" 
                    }}>
                      {item.status}
                    </span>
                  </td>
                  <td style={styles.td}>
                    <input 
                      type="text" 
                      placeholder="Add logging remarks..." 
                      value={item.remarks} 
                      onChange={(e) => updateRowProperty(item.uid, "remarks", e.target.value)} 
                      style={{ ...styles.formInlineInput, padding: "6px" }} 
                    />
                  </td>
                </tr>

                {/* Sliding Checklist Checkbox Row Block (appears right below when clicked) */}
                    ...
                    <td colSpan="8" style={{ backgroundColor: "#f8fafc", padding: "16px 24px", borderBottom: "1px solid #cbd5e1" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        <div style={{ fontSize: "12px", fontWeight: "700", color: "#475569", textTransform: "uppercase" }}>
                          Parameter Verification Checklist for {item.modelNo}:
                        </div>
                            ...
                            return (
                              <label key={pIdx} style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "13px", cursor: "pointer", color: "#0f172a" }}>
                                <input 
                                  type="checkbox" 
                                  checked={isChecked} 
                                  onChange={(e) => handleToggleChecklistCheckbox(item.uid, checkpoint, e.target.checked)}
                        ...
                        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "6px" }}>
                          <button 
                            type="button" 
                            onClick={() => setExpandedChecklistUid(null)} 
                            style={{ ...styles.actionInlineBtn, backgroundColor: "#475569", color: "#ffffff" }}
                          >
                            Collapse View
                          </button>
                        </div>
